...
const run = async (client, msg, args) => {
    ...
    pages = pages.map((page, i) => {
        ...
        let embed = new MessageEmbed()
            ...
            .setDescription(
                (page.map((user, j) => `**${j+1+i*10}.** ${client.users.cache.get(user.id).toString()} - **${Object.values(user.invites).reduce((x,y)=>x+y).toLocaleString()} ${translate("Davetler", "invites")}** (**${user.invites.normal}** ${translate("Gecerli", "regular")}, **${user.invites.left}** ${translate("Çıkanlar", "left")}, **${user.invites.fake}** ${translate("Sahteler", "fake")}, **${user.invites.bonus}** bonus)`)
                .join("\n")) || "***Hiçbir üye derecelendirilmemiştir.***"
            ).setFooter(`${translate("Sordu:", "Asked by:")} ${msg.member.nickname || msg.author.username} • Page ${(i+1).toLocaleString()}/${pages.length}`, msg.author.displayAvatarURL())
        var messageButtons = [];
...
module.exports = {
    aliases: ["lb"],
    description: "Davetlerin sıralamasını döndürür.",
    run: run
};